    ...
    <el-container>
      ...
      <!-- 内容容器 -->
          ...
          <el-footer>
           ...
           <!-- 新增和编辑是同一个弹出框 -->
          <el-dialog title="查看详情" :close-on-click-modal="false" top="5vh" v-dialogDrag width="1000px" :visible.sync="dialogFormVisible">
              <!-- 选项卡 -->
          ...
          <!-- 选项卡表格 -->
            <el-form :rules="rules" ref="dataForm" :model="temp" label-width="110px">
              ...
              <el-form-item label="交付日期" prop="payTime" style="margin-left:200px">
               ...
               <el-row>
              <el-col :span="11">     
              <el-form-item label="质保验收完成日期" prop="warrantyAcceptanceTime" label-width="140px">
               <div class="block">
                <el-date-picker
                  style="width:260px"
                  ...
                  format="yyyy-MM-dd"
                  value-format="yyyy-MM-dd"
                   :disabled='kostyp?true:false'>
                </el-date-picker>
              </div>
              </el-form-item>
              </el-col>
              <el-col :span="13">
              <el-form-item label="验收情况" prop="acceptanceSituation" style="margin-left:80px" :disabled='kostyp?true:false'> 
                <el-radio v-model="temp.acceptanceSituation" :disabled='kostyp?true:false' label="1">验收通过</el-radio>
                <el-radio v-model="temp.acceptanceSituation" :disabled='kostyp?true:false' label="0">验收不通过</el-radio>
            ...
            <el-button type="primary" @click="referringData" size="mini" v-show="submitlist">提交</el-button>
...
<script>
import { fetchList, smallAblist, samllDelete, uploadlist, circuList, updateAuditing, updeletedata, insertFile, getCurrentUser, getNodeTask, newlYupdate } from '@/api/equipMent/pfixSeconduct'
...
export default {
  ...
  data() {
    return {
      ...
      retrnsty: false, // 退回按钮显隐
      submitlist: false, // 提交按钮显隐
...
// ******* 文件预览结束**********
    ...
    checkDetails(row) {
      ...
      this.porisgid = row.processInstanceId// 流转
      ...
      if (row.taskName == '调整申请') {
        ...
        this.consent = true // 同意按钮显隐
        this.retrnsty = true// 退回按钮显隐
        this.submitlist = false// 提交按钮显隐
        this.kostyp = true// 禁止的input
      }
      ...
      this.$refs['dataForm'].validate(valid => {
        if (valid) {
          ...
          const tempData = {
            ...
            warrantyAcceptanceTime: this.temp.warrantyAcceptanceTime
          }
          if (this.smallslist == '') {
            this.$notify({
              title: '提示',
