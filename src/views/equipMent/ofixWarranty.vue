       ...
       <!-- 顶部搜索 -->
        ...
        <div class="filter-container">
            ...
            <el-form :inline="true">
                ...
                <el-radio v-model="listQuery.acceptanceType" label="0">验收不通</el-radio>
                </el-form-item>
             
                <el-form-item label="审核状态">
                    <el-select v-model="listQuery.auditingType" style="width:205px" clearable  placeholder="请选择">
                    <el-option
          ...
          <el-main style="padding:20px 0px">
            <el-table :data="listyst" v-loading="listLoading" border fit highlight-current-row
              ...
              <el-table-column prop="useFactoryName" align="center" label="使用工厂" min-width="80">
               ...
               <el-table-column prop="useDeptName" align="center" label="使用部门" min-width="80">
                </el-table-column>
               ...
               <el-table-column prop="warrantyAcceptanceTime" align="center" label="质保验收完成日期" min-width="80">
                 </el-table-column>
              ...
              <el-table-column align="center" label="操作" min-width="150" fixed="right" class-name="small-padding fixed-width"> <!-- v-if="edit"  v-if="afreshst" -->
                <template slot-scope="scope">
                  <el-button title="编辑" type="primary" :disabled="scope.row.stateshow?true:false" size="mini" icon="el-icon-edit" @click="handleUpdate(scope.row)" ></el-button>
          ...
          <!-- 新增和编辑是同一个弹出框 -->
          <el-dialog :title="textMap[dialogStatus]" :close-on-click-modal="false" v-dialogDrag top="5vh" width="1000px" :visible.sync="dialogFormVisible">
             <!-- 选项卡 -->
            <el-tabs v-model="activeName" v-loading="fromLoading" @tab-click="fathClick">
             <el-tab-pane label="表单信息" name="first">
              ...
              <el-form-item>
                <span class="equip-title">设备质保验收单</span>
              </el-form-item>
              ...
              <el-input style="width:290px" v-model="temp.useFactoryName" :disabled='siteCodeState?true:false' class="enter medium-input" placeholder="输入使用工厂" ></el-input>
               ...
               <el-col :span="16">
              <el-form-item label="使用部门" prop="useDeptName" style="margin-left:200px;"> 
              <el-input v-model="temp.useDeptName" :disabled='siteCodeState?true:false' style="width:290px" class="enter medium-input" placeholder="请输入使用车间" ></el-input>
              </el-form-item>
              </el-col>
              </el-row>
              ...
              <el-form-item label="交付日期" prop="payTime" style="margin-left:200px">
               ...
               <el-row>
              <el-col :span="11">     
              <el-form-item label="质保验收完成时间" prop="warrantyAcceptanceTime" label-width="140px" style="margin-left:0px">
               <div class="block">
                <el-date-picker
                  style="width:260px"
              ...
              <el-col :span="13">
              <el-form-item label="验收情况" prop="acceptanceSituation" style="margin-left:80px"> 
                <el-radio v-model="temp.acceptanceSituation" label="1" :disabled='!flieshow'>验收通过</el-radio>
                <el-radio v-model="temp.acceptanceSituation" label="0" :disabled='!flieshow'>验收不通过</el-radio>
                </el-form-item>
              </el-col>
              </el-row>
              ...
              <el-form-item label="设备质保验收单附件" class="sepost" label-width="140px" style="margin-left:10px">
              ...
              <input id="uploadImg" type="file" @change="fileUpload" v-show="flieshow">
              ...
              <!-- <input class="uploadInput" id="uploadImg" ref="uploadImg" type="file" accept="image/gif,image/jpg,image/png,/image/gif,image/jpeg" @change="noticeUpload"> -->
               ...
               <input id="twoupload" type="file" @change="twofileUp" v-show="flieshow"> 
...
<script>
import { fetchListdsy, updateArticle, fetchPv, evaluate, circuList, uploadlist, smallAblist, createArticlest, deleteapp, updeletedata, insertFile, getCurrentUser, fixedascode } from '@/api/equipMent/ofixWarranty'
import { flowsheet } from '@/api/equipMent/acceptanceSheet'
import waves from '@/directive/waves' // 水波纹指令
import { fetchButton } from '@/api/common/button'
import { getProxyOrgList,choosePersonList, fetchChooseTree, transferReplace } from '@/api/common/common'// 代理组织
...
export default {
  ...
  data() {
    return {
      ...
      // 弹框内输入框不填显示的提示信息  warrantyAcceptanceTime  acceptanceSituation
      rules: {
   ...
   handlePreview(row){
    ...
    searchsty() {
      ...
      if (this.value13 == null) {
        this.listQuery.warrantyAcceptanceTime = ''
        this.listQuery.warrantyAcceptanceTime = ''
      } else {
        this.listQuery.warrantyAcceptanceTime = this.value13[0]// 开始时间
        this.listQuery.warrantyAcceptanceTime = this.value13[1]
      }
    },
    ...
    saveData() {
      this.$refs['dataForm'].validate(valid => {
        if (valid) {
          ...
          this.temp.useDept = this.useDept
          ...
          this.temp.list = op
          //  this.temp.acceptanceFinishTime = row.acceptanceFinishTime
          this.fromLoading = true
          if (this.smallslist == '') {
            this.$notify({
              title: '提示',
    ...
    handleUpdate(row) {
      ...
      /*  this.getlistsamll()//第一个小表格数据list
      this.getlisttwo()//第二个小表格数据list */
