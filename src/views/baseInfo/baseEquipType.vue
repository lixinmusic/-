      ...
      <el-container>
        ...
        <el-main style="padding-top:0;">
          ...
          <el-table :key='tableKey' :data="listData" height='200' v-loading="listLoading" border fit highlight-current-row
            ...
            <el-table-column prop="updateTime" align="center" label="修改时间" min-width="50">
              <template slot-scope="scope">
                <span>{{scope.row.updateTime | parseTime('{y}-{m}-{d} {h}:{i}:{s}')}}</span>
              </template>
            </el-table-column>
            <el-table-column prop="updateUser" align="center" label="修改人" min-width="50">
            </el-table-column>
            ...
            <el-row :gutter="20">
              ...
              <el-col :span="12">
                <el-form-item label="修改时间" prop="updateTime">
                  <el-date-picker v-model="popData.updateTime" style="width:100%" type="datetime" disabled>
                  </el-date-picker>
                </el-form-item>
              </el-col>
            </el-row>
            <el-row>
              <el-col :span="24">
                <el-form-item label="修改人" prop="updateUser">
                  <el-input v-model="popData.updateUser" disabled></el-input>
                </el-form-item>
              </el-col>
            </el-row>
            <el-row>
...
<script>
import { fetchTree, fetchListByCode, deleteCode, createArticle, fetchDetail, updateArticle, getCurrentUser } from '@/api/baseInfo/baseEquipType'
  ...
  methods: {
    ...
    deleteHandle(row) {
      this.$confirm('是否确认删除?', '提示', {
        ...
        deleteCode(categoryCode).then(response => {
          if (response.data.code === 200) {
            this.getList(this.parCode)
            this.getTree()
            this.$message({
    ...
    handleCreate() {
      ...
      if (this.treeData[0].children.length == 0 && this.treeData[0].parentCategoryCode != 'SBLX' && this.treeData[0].categoryCode != 'SBLX') {
        this.$notify({
          title: '警告',
          message: '选中设备名称不能新增',
          type: 'warning',
      ...
      getCurrentUser().then(response => {
        this.popData.updateUser = response.data.data.perName
      })
    },
    createData() {
    ...
    handleUpdate(row) {
      ...
      // this.isDisable = true
      const categoryCode = row.categoryCode
      fetchDetail(categoryCode).then(response => {
        this.popData = Object.assign({}, response.data.data.equipCategory)
        this.popData.updateTime = new Date(this.popData.updateTime)
        getCurrentUser().then(response => {
          this.popData.updateUser = response.data.data.perName
        })
      })
    ...
    updateData() {
      this.$refs['dataForm'].validate(valid => {
        if (valid) {
          ...
          tempData.updateTime = +new Date(tempData.updateTime) // 改变时间：2017年11月30日16:41:05 GMT+0800（CST）至1512031311464
