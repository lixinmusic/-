        ...
        <div class="filter-container">
            ...
            <el-form :inline="true">
                ...
                <el-form-item label="审核状态">
                    <el-select v-model="listQuery.checkStatus" style="width:190px" clearable  placeholder="请选择">
                    <el-option
                ...
                <el-form-item label="验收情况" prop="warrantyType"> 
                <el-radio v-model="listQuery.warrantyType" label="0">验收通过</el-radio>
                <el-radio v-model="listQuery.warrantyType" label="1">验收不通过</el-radio>
                <el-radio v-model="listQuery.warrantyType" label="">全部</el-radio>
                </el-form-item>
          ...
          <el-main style="padding:20px 0px">
            ...
            <el-table :data="listyst" v-loading="listLoading" border fit highlight-current-row
              style="width: 100%;min-height:auto;margin-top:0px" height="500" :header-cell-style="{background:'oldlace'}">
               ...
               <el-table-column prop="formCode" align="center" label="单据编码"  min-width="100">
              ...
              <el-table-column prop="factoryName" align="center" label="使用工厂" min-width="80">
               ...
               <el-table-column prop="evaluateTime" align="center" label="评价日期" min-width="80">
                  <template slot-scope="scope">
                  {{scope.row.evaluateTime!==null?scope.row.evaluateTime.substring(0,10):''}}
                  </template> 
                 </el-table-column>
               <el-table-column prop="acceptanceSituationName" align="center" label="验收情况" min-width="80">
                 </el-table-column>
               <el-table-column prop="checkStatusName" align="center" label="审核状态" min-width="80">
                 </el-table-column>
          ...
          <el-dialog :title="textMap[dialogStatus]" :close-on-click-modal="false" v-dialogDrag top="8vh" width="1000px" :visible.sync="dialogFormVisible" >
            ...
            <el-tabs v-model="activeName" v-loading="fromLoading" @tab-click="fathClick">
             <el-tab-pane label="表单信息" name="first">
              ...
              <el-form-item>
                <span class="equip-title">设备改造验收单</span>
              </el-form-item>

              <el-row>
              <el-col :span="8">     
              <el-form-item label="单据编码"  prop="formCode">
              <el-input style="width:290px" v-model="temp.formCode" disabled  class="enter medium-input" placeholder="空" ></el-input>
              </el-form-item>
              </el-col>
               <el-col :span="16">
              ...
              <el-input style="width:290px" v-model="temp.supplier" :disabled='siteCodeState?true:false' class="enter medium-input" placeholder="输入改造供应商" ></el-input>
              </el-form-item>
              </el-col>
               <el-col :span="16">
              <el-form-item label="使用公司" prop="companyName" style="margin-left:200px;"> 
              <el-input v-model="temp.companyName" :disabled='siteCodeState?true:false' style="width:290px" class="enter medium-input" placeholder="请输入使用公司" ></el-input>
              </el-form-item>
              </el-col>
              </el-row>
              ...
              <el-input v-model="temp.factoryName" :disabled='siteCodeState?true:false' style="width:290px" class="enter medium-input" placeholder="请输入使用工厂" ></el-input>
              </el-form-item>
              </el-col>
              <el-col :span="16">
              <el-form-item label="使用车间" prop="divisionName" style="margin-left:200px;">
              <el-input v-model="temp.divisionName" :disabled='siteCodeState?true:false' style="width:290px" class="enter medium-input" placeholder="请输入使用车间" ></el-input>
              </el-form-item>
              </el-col>
              </el-row>
              ...
              <el-row>
              <el-form-item label="改造内容" prop="context" label-width="120px">
               <el-input type="textarea" v-model="temp.context" :disabled='siteCodeState?true:false' style="width:798px" maxlength="128" :autosize="{ minRows: 2, maxRows: 4}" placeholder="请输入改造内容">
               </el-input>
              </el-form-item>
              </el-row>
             ...
             <el-col :span="10">
              <el-form-item label="验收情况" prop="warrantyType"> 
                <el-radio v-model="temp.warrantyType" label="0" :disabled='!filshow'>验收通过</el-radio>
                <el-radio v-model="temp.warrantyType" label="1" :disabled='!filshow'>验收不通过</el-radio>
                </el-form-item>
              </el-col>
              <el-col :span="14">     
              <el-form-item label="评价日期" prop="evaluateTime" style="margin-left:120px">
               <div class="block">
                <el-date-picker
                  style="width:280px"
              ...
              <el-row>
              <el-col :span="18">
              <span style="color:red;position:absolute;bottom:30px;left:10px">*</span>
              <el-form-item label="改造验收单附件" class="sepost" label-width="130px">
              ...
              <el-form-item label="设备改造记录附件" class="sepost" label-width="140px">
...
<script>
import { fetchListdsy, updateArticle, evaluate, producePv, circuList, uploadlist, smallAblist, createArticlest, deleteapp, updeletedata, insertFile, getCurrentUser, fixedascode, transferReplace } from '@/api/equipRemould/remakeCheck'
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        evaluateTime: [{ required: true, message: '请选择', trigger: 'change' }],
        warrantyType: [{ required: true, message: '请选择', trigger: 'change' }]
  ...
  methods: {
      ...
      fetchListdsy(this.listQuery).then(response => {
        if (response.data.code == 200) {
          ...
          for (var i = 0; i < this.listyst.length; i++) {
            if (this.listyst[i].warrantyType == 0) {
              this.listyst[i].acceptanceSituationName = '验收通过'
            } else if (this.listyst[i].warrantyType == 1) {
              this.listyst[i].acceptanceSituationName = '验收不通过'
            }

		 if (this.listyst[i].checkStatus == 2) {
              this.listyst[i].checkStatusName = '审核中'
              this.listyst[i].stateshow = true
              this.listyst[i].deleshow = true
              this.listyst[i].stshow = true
              console.log(111)
            } else if (this.listyst[i].checkStatus == 1) {
              this.listyst[i].checkStatusName = '未提交'
              this.listyst[i].stateshow = false
              this.listyst[i].deleshow = false
              this.listyst[i].stshow = false
		   } else if (this.listyst[i].checkStatus == 3) {
              this.listyst[i].checkStatusName = '审核通过'
              this.listyst[i].stateshow = true
              this.listyst[i].deleshow = true
              this.listyst[i].stshow = true
		   } else if (this.listyst[i].checkStatus == 4) {
              this.listyst[i].checkStatusName = '审核不通过'
              this.listyst[i].stateshow = true
              this.listyst[i].deleshow = true
              this.listyst[i].stshow = true
            } else if (this.listyst[i].checkStatus == 5) {
              this.listyst[i].checkStatusName = '流程关闭'
              this.listyst[i].stateshow = true
              this.listyst[i].deleshow = true
              this.listyst[i].stshow = true
            }

            if (this.listyst[i].warrantyType !== 0 || this.listyst[i].checkStatus !== 3) {
              this.listyst[i].newshow = true
              if (this.listyst[i].isCreateWarranty == 0 && this.listyst[i].warrantyType == 1 && this.listyst[i].checkStatus == 3) {
                this.listyst[i].produceshow = false
              } else if (this.listyst[i].isCreateWarranty == 1) {
                this.listyst[i].produceshow = true
              }
            }
            if (this.listyst[i].warrantyType !== 1 || this.listyst[i].checkStatus !== 3) {
              this.listyst[i].produceshow = true
              if (this.listyst[i].isReAcceptance == 0 && this.listyst[i].warrantyType == 0 && this.listyst[i].checkStatus == 3) {
      ...
      fixedascode(this.fixecodelist).then(response => {
        if (response.data.code == 200) {
          ...
          for (var i = 0; i < this.fixelistcode.length; i++) {
            ...
            if (this.fixelistcode[i].acceptanceSituation == 0) {
              this.fixelistcode[i].acceptanceSituationName = '验收不通过'
            } else if (this.fixelistcode[i].acceptanceSituation == 1) {
              this.fixelistcode[i].acceptanceSituationName = '验收通过'
            }
          }
    ...
    searchsty() {
      ...
      if (this.value13 == null) {
        this.listQuery.evaluateTimeStart = ''
        this.listQuery.evaluateTimeEnd = ''
      } else {
        this.listQuery.evaluateTimeStart = this.value13[0]// 开始时间
        this.listQuery.evaluateTimeEnd = this.value13[1]
      }
    },
    // 删除事件
    ...
    checkDetails(row) {
      ...
      if (row.warrantyType !== null) {
        this.temp.warrantyType = row.warrantyType.toString()
      }
      this.circulationlist()
      // 第一个小表格接口
      const samllblist = {
        moduleType: 23,
        attachmentCode: 'SBGZYSD',
    ...
    newlyBtn(row) {
      ...
      this.temp.useWorkshopName = row.useWorkshopName
      this.temp.equipmentName = row.equipmentName
      this.temp.typeSpecification = row.typeSpecification
      this.temp.outFactoryCode = row.outFactoryCode
      this.temp.fixedAssetCode = row.fixedAssetCode
      this.temp.supplier = row.supplier
      this.temp.payTime = row.payTime
      this.cheid = row.equId
      this.newId = row.id// id赋值
      this.useCompany = row.useCompany// 使用公司
      this.useFactory = row.useFactory// 使用工厂
      this.useWorkshop = row.useWorkshop// 使用车间
      this.fromLoading = false// 弹窗加载动画
    ...
    saveData(row) {
      this.$refs['dataForm'].validate(valid => {
        if (valid) {
          ...
          this.temp.equId = this.cheid
          this.temp.id = this.newId// id
          this.temp.useCompany = this.useCompany
          this.temp.useFactory = this.useFactory
          this.temp.useWorkshop = this.useWorkshop
          const op = this.smallslist
          const lo = this.towlistdata
          for (var i = 0; i < lo.length; i++) { // 俩个小表格用一个list传给后台
            op.push(lo[i])
          }
          this.temp.list = op
          console.log(234555)
          console.log(this.temp.list)
          this.fromLoading = true// 弹窗加载动画
          if (this.smallslist == '') {
            this.$notify({
              title: '提示',
    ...
    handleUpdate(row) {
      ...
      evaluate(ids).then(response => {
        ...
        if (this.popTitle.warrantyType !== null) {
          this.temp.warrantyType = this.popTitle.warrantyType.toString()
        } else if (this.popTitle.warrantyType == 2) {
          this.temp.warrantyType = ''
        }
