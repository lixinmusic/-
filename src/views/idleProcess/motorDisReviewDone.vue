          ...
          <el-tab-pane label="表单信息" name="first">
            <el-form :rules="rules" ref="dataForm" :model="temp" label-position="left" label-width="100px">
              ...
              <el-table :data="temp.equipMachineInfoList" ref="dataForm" :rules="rules" tooltip-effect="dark" border stripe 
                ...
                <el-table-column width="60px" v-if="false">
                  <template slot-scope="scope">
                    <span style="color:#409EFF;font-size:16px" @click.prevent="addRow()"><i class="el-icon-plus"></i></span>
                    <span style="color:red;font-size:16px" @click.prevent="delRow(scope.row)"><i class="el-icon-minus"></i></span>
                  </template>
                </el-table-column>
...
<script>
import { fetchDoneList, getDetail, fetchPrint } from '@/api/idleProcess/motorDisReview'
import { commonOptions1, recordList, processChart } from '@/api/common/common'
export default {
  name: 'motorDisReviewDone',
  data() {
    return {
      chartData: null, // 流程图
      // 表格数据
