  ...
  <div class="dashboard-container">
    <el-row :gutter="20">
      <el-col :span="12">
        <el-card shadow="hover" :body-style="{height: '380px'}">
          <div slot="header" class="clearfix">
            <span style="color:#0067FF;font-weight:800;">我的审批</span>
          </div>
          <div style="border-bottom:1px black solid;padding-bottom:5px;">
            <span>待办</span><span class="red-point">&nbsp;{{handleTypeNums}}&nbsp;</span>
            <el-button style="float: right; padding: 3px 0" type="text" @click="moreApproval()">更多&nbsp;&nbsp;<i class="el-icon-d-arrow-right"></i></el-button>
          </div>
          <el-table :data="approvalList" :show-header="false" height="380" style="width: 100%;font-size:15px;" v-loading="listLoading">
            <el-table-column :show-overflow-tooltip="true">
              <template slot-scope="scope">
                <span class="look-detail" @click="approvalDetail(scope.row)">{{scope.row.title}}</span>
              </template>
            </el-table-column>
      ...
      <el-col :span="12">
        <el-card shadow="hover" :body-style="{height: '380px'}">
          <div slot="header" class="clearfix">
            <span style="color:#0067FF;font-weight:800;">我的消息</span>
          </div>
          <div style="border-bottom:1px black solid;padding-bottom:5px;">
            <span>未读消息</span><span class="red-point">&nbsp;{{isReadNums}}&nbsp;</span>
            <el-button style="float: right; padding: 3px 0" type="text" @click="moreMessage()">更多&nbsp;&nbsp;<i class="el-icon-d-arrow-right"></i></el-button>
          </div>
          <el-table :data="messageList" :show-header="false" height="380" style="width: 100%;font-size:15px;" v-loading="listLoading">
            <el-table-column :show-overflow-tooltip="true">
              <template slot-scope="scope">
    ...
    <el-dialog :title="messageTitle" :close-on-click-modal="false" :visible.sync="centerDialogVisible" width="30%">
      ...
      <span slot="footer" class="dialog-footer">
        <el-button type="primary" @click="updateData()">知道了</el-button>
    ...
    <el-dialog :title="messageTitle" :close-on-click-modal="false" :visible.sync="formDialogVisible" width="30%">
      <!-- <div>{{equipMessageContent}}</div> -->
      <div v-if="equipMessageContent.length>0" v-for="item in equipMessageContent" :key="item.remindId">
        <span>{{item.content}}</span>
      </div>
      <el-form :model="temp1" label-position="left" label-width="110px">
        <el-row>
          <el-col :span="24">
            <el-form-item label="按要求交货" prop="isDelivery">
              <el-radio v-model="temp1.isDelivery" label="1" @change="radioChange()">是</el-radio>
              <el-radio v-model="temp1.isDelivery" label="0">否</el-radio>	
            </el-form-item>
          </el-col>
        </el-row>
        <el-row>
          <el-col :span="24">
            <el-form-item label="下次交货日期" prop="nextDeliveryDate">
              <el-date-picker style="width:100%"
                v-model="temp1.nextDeliveryDate"
                type="date"
                placeholder="选择时间"
                format="yyyy-MM-dd"
                value-format="yyyy-MM-dd"
                 :disabled="temp1.isDelivery==1?true:false">
              </el-date-picker>
            </el-form-item>
          </el-col>
        </el-row>
        <el-row>
          <el-col :span="24">
            <el-form-item label="处理结果" prop="processResult">
              <el-input type="textarea" maxlength="200" v-model="temp1.processResult"></el-input>
            </el-form-item>
          </el-col>
        </el-row>
      </el-form>
      <span slot="footer" class="dialog-footer">
        <el-button type="primary" @click="equipOK()">确认</el-button>
...
<script>
import { getFiveFormList, getFiveMessageList, updateMessage, equipmessage, updateFormMessage } from '@/api/common/approvalMessages'
...
export default {
  ...
  methods: {
    getList() {
      ...
      getFiveFormList().then(response => {
        if (response.data.code === 200) {
          if (response.data.data != null) {
            this.handleTypeNums = response.data.data.handleTypeNums
            this.approvalList = response.data.data.list
            getFiveMessageList().then(response => {
              if (response.data.code === 200) {
                if (response.data.data != null) {
                  this.isReadNums = response.data.data.isReadNums
    ...
    approvalDetail(row) {
      switch (row.moduleType) {
        ...
        case 29: // 退货出厂
          ...
          break
        case 30: // 设备采购备案
          this.$router.push({ path: '/equipMent/procurementFilingNeed', name: 'procurementFilingNeed' })
          break
        case 31: // 闲置设备出厂单
    ...
    messageDetail(row) {
      ...
      if (row.messageType == 1) {
        ...
        this.temp1.messageId = row.messageId
        this.temp1.userMessageId = row.id
        this.equipMessageContent = JSON.parse(row.content)
        this.temp1.id = this.equipMessageContent[0].remindId
    ...
    equipOK() {
      equipmessage(this.temp1).then(response => {
        if (response.data.code == 200) {
    ...
    radioChange() {
      console.log(this.temp1.isDelivery)
      this.temp1.nextDeliveryDate = undefined
    },
    // 根据编号查看
    handleDetail() {
      // 根据id获取详情
