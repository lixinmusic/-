  ...
  <div class='app-container'>
    <el-container>
        ...
        <el-table :key='tableKey' :data="tableData" height="200" row-key="id" v-loading="listLoading" 
          ...
          <el-table-column align="center" label="操作" width="240px" fixed="right">
            <template slot-scope="scope">
              ...
              <el-button title="生成采购申请单" type="primary" size="mini" icon="el-icon-document" @click="handleProduces(scope.row)" :disabled="scope.row.produceDisabled?true:false"></el-button>
              <el-button title="查看采购申请单" type="primary" icon="el-icon-view" size="mini"  @click="handleLook(scope.row)" :disabled="scope.row.lookDisabled?true:false"></el-button>
            </template>
          </el-table-column>
        </el-table>
      </el-main>
      ...
      <el-dialog :title="textMap[dialogStatus]" :close-on-click-modal="false" :visible.sync="dialogFormVisible"
        width="80%" v-dialogDrag v-loading="listLoading">
        <el-tabs v-model="activeName" @tab-click="handleClick">
          <el-tab-pane label="表单信息" name="first">
            <el-form :rules="rules" ref="dataForm" :model="temp" label-position="left" label-width="135px">
              ...
              <el-row :gutter="20">
                <el-col :span="11">
                  <el-form-item label="使用地址" prop="useAddress">
                    <el-input v-model="temp.useAddress" maxlength="30" :disabled="!chooseShow"></el-input>
                  </el-form-item>
                </el-col>
                <el-col :span="12">
                  <el-form-item label="设备采购建议" prop="equipBuySuggest">
                    <el-radio v-model="temp.equipBuySuggest" label="内部制造" :disabled="!chooseShow">内部制造</el-radio>
                    <el-radio v-model="temp.equipBuySuggest" label="外部制造" :disabled="!chooseShow">外部制造</el-radio>	
                  </el-form-item>
                </el-col>
              </el-row>
              <el-row :gutter="20">
              ...
              <el-row :gutter="20">
                <!-- <el-col :span="12">
                  <el-form-item label="设备改造(填上原设备编码)" prop="equipReform">
                    <el-input v-model="temp.equipReform" maxlength="200" :disabled="!chooseShow"></el-input>
                  </el-form-item>
                </el-col> -->
                <el-col :span="12">
                  <!-- 需要集团技改办主任填写预算节点 -->
              ...
              <!-- <el-form-item label="工厂设备科长(安环内容)">
                <el-input type="textarea" maxlength="128" v-model="temp.discription"></el-input>
              </el-form-item>
              <el-form-item label="资产部安环科意见">
                <el-input type="textarea" maxlength="128" v-model="temp.discription"></el-input>
              </el-form-item>
              <el-form-item label="工厂厂长意见">
                <el-input type="textarea" maxlength="128" v-model="temp.discription"></el-input>
              </el-form-item>
              <el-form-item label="设备副总意见">
                <el-input type="textarea" maxlength="128" v-model="temp.discription"></el-input>
              </el-form-item> -->
            </el-form>
      ...
      <el-dialog title="提示" :close-on-click-modal="false" :visible.sync="centerDialogVisible" width="30%">
        <span>已生成采购单，请到采购申请单页面处理</span>
        <span slot="footer" class="dialog-footer">
          <el-button type="primary" @click="centerDialogVisible=false">确定</el-button>
        </span>
      </el-dialog>
      <!-- 查看采购申请单 -->
      <el-dialog title="查看生成采购单" :close-on-click-modal="false" :visible.sync="dialogTableVisible">
        <el-table :data="gridData">
          ...
          <el-table-column prop="purchaseApplicationCode" align="center" label="生成单号" >
            <template slot-scope="scope">
              <span>{{scope.row.purchaseApplicationCode}}</span>
            </template>
          </el-table-column>
          <el-table-column prop="ctime" align="center" label="生成日期">
            <template slot-scope="scope">
...
<script>
import { fetchList, saveArticle, createArticle, getDetail, updateArticle, updateArticleSubmit, delById, fetchDetail, fetchProduce, findPurchaseByRecordCode, fetchPrint } from '@/api/equipRecord/equipRecordNotice'
...
export default {
  ...
  methods: {
          ...
          if (response.data.data != null && response.data.data.list != null) {
            ...
            for (var i = 0; i < this.tableData.length; i++) {
              if (this.tableData[i].auditingType == 1) {
                ...
                this.tableData[i].produceDisabled = true
                ...
                const oldYear = this.tableData[i].requirementCompleteTime.substring(0, 4)
                const oldMonth = this.tableData[i].requirementCompleteTime.substring(5, 7)
                const oldDay = this.tableData[i].requirementCompleteTime.substring(8, 10)
                if (this.year < oldYear) {
                  this.tableData[i].produceDisabled = false
                } else if (this.year <= oldYear && this.month < oldMonth) {
                  this.tableData[i].produceDisabled = false
                } else if (this.year <= oldYear && this.month <= oldMonth && this.day <= oldDay) {
                  this.tableData[i].produceDisabled = false
                } else {
                  this.tableData[i].produceDisabled = true
                }
    ...
    handleUpdate(row) {
      ...
      this.activeName = 'first'
      this.dialogFormVisible = true
      this.documentWordCode = row.recordCode
      const id = row.id
      // 根据id获取详情
    ...
    handleProduces(row) {
      const params = {
        ...
        // dctualOrgCode: row.dctualOrgCode,
        // dctualOrgName: row.dctualOrgName
        proxyOrgCode: row.proxyOrgCode
      }
      fetchProduce(params).then(response => {
        if (response.data.code === 200) {
    ...
    handleLook(row) {
      ...
      const recordCode = row.recordCode
      findPurchaseByRecordCode(recordCode).then(response => {
        this.gridData = response.data.data
        this.id = this.gridData[0].id
        this.purchaseApplicationCode = this.gridData[0].purchaseApplicationCode
        this.ctime = this.gridData[0].ctime
      })
    },
    // 获取文件列表
    getFileList() {
