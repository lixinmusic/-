...
import { endProcess, recordList, getCurrentTask, processChart, fetchChooseTree, choosePersonList, transferList } from '@/api/common/common'
export default {
  name: 'equipRepairNeed',
  data() {
    return {
      chartData: null, // 流程图
      // 节点判断是否可输入
      ...
      getCurrentTask(this.processInstanceId).then(response => {
        if (response.data.code === 200) {
          ...
          if (this.nodeName == '调整申请') { // 判断是否为调整申请
            this.tzsqTask = false
          } else if (this.nodeKey == 'repairOut_assetdepartment_vp') { // 资产部副部长
