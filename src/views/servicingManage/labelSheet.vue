        ...
        <div class="filter-container">
            ...
            <el-form :inline="true">
                ...
                <el-form-item label="审核状态">
                    <el-select v-model="listQuery.checkStart" style="width:189px" clearable  placeholder="请选择">
                    <el-option
            ...
            <el-table :data="listyst" v-loading="listLoading" border fit highlight-current-row
              ...
              <el-table-column align="center" label="操作" min-width="160" fixed="right" class-name="small-padding fixed-width">   <!-- v-if="edit"  v-if="afreshst" v-if="speechst" -->
                ...
                <!--   <el-button title="重新验收" type="warning" :disabled="scope.row.newshow?true:false" icon="el-icon-refresh" size="mini"  @click="newlyBtn(scope.row)" v-if="afreshst"></el-button>
                  ...
                  <el-button title="生成设备维修质保确认单" type="primary" :disabled="scope.row.produceshow?true:false" icon="el-icon-document" size="mini" @click="newlproDuce(scope.row)" v-if="speechst"></el-button> -->
                </template>
              </el-table-column>
            </el-table>
          </el-main>
          ...
          <el-dialog :title="textMap[dialogStatus]" v-dialogDrag top="8vh" width="1000px" :visible.sync="dialogFormVisible" >
            ...
            <el-tabs v-model="activeName" v-loading="fromLoading" @tab-click="fathClick">
             <el-tab-pane label="表单信息" name="first">
              ...
              <el-form-item>
                <span class="equip-title">设备维修验收记录单</span>
              </el-form-item>
              ...
              <el-input style="width:290px" v-model="temp.factoryName" :disabled='siteCodeState?true:false' class="enter medium-input" placeholder="输入使用工厂" ></el-input>
              </el-form-item>
              </el-col>
               <el-col :span="16">
              ...
              <el-input v-model="temp.serialNumber" :disabled='siteCodeState?true:false' style="width:290px" class="enter medium-input" placeholder="请输入出厂编号" ></el-input>   
              </el-form-item>
              </el-col>
              <el-col :span="16">
              ...
              <el-input v-model="temp.buyers" :disabled='siteCodeState?true:false' style="width:290px" class="enter medium-input" placeholder="请输入供应商" ></el-input>   
              </el-form-item>
              </el-col>
              <el-col :span="16">
              <el-form-item label="交付日期" prop="deliverDate" style="margin-left:200px">
               ...
               <el-row>
              <el-col :span="10">     
              <el-form-item label="验收完成时间" prop="acceptanceDate" >
               <div class="block">
                <el-date-picker
                  style="width:290px"
              ...
              <el-row>
              <el-col :span="18">
              <el-form-item label="设备维修验收记录单" class="sepost" label-width="140px">
            ...
            <el-button type="primary" @click="sheetData" v-show="fastener1" size="mini">选择设备</el-button>
            </el-form-item>
            </el-form>
...
<script>
import { fetchListdsy, updateArticle, fetchPv, evaluate, producePv, circuList, uploadlist, smallAblist, createArticlest, deleteapp, updeletedata, insertFile, getCurrentUser, fetchsList } from '@/api/servicingManage/labelSheet'
...
export default {
  ...
  data() {
    return {
      ...
      statustrue: true, // 新增提交
      statusfalse: false, // 新增保存
      list1: [],
      list2: [],
  ...
  methods: {
        ...
        if (response.data.code == 200) {
          ...
          for (var i = 0; i < authority.length; i++) {
            if (authority[i].buttonName == '新增') {
              ...
              this.cutout = true
            } else if (authority[i].buttonName == '生成设备维修质保确认单') {
              this.speechst = true
            }
          }
        } else {
    ...
    getAA() {
      ...
      fetchListdsy(this.pageNum, this.pageSize, this.listQuery).then(response => {
        ...
        for (var i = 0; i < this.listyst.length; i++) {
          ...
          } else if (this.listyst[i].acceptanceInfo == 3) {
            this.listyst[i].acceptanceInfoName = ''
          }
          ...
          // 判断删除
          if (this.listyst[i].checkStart == 1 && this.listyst[i].isCanDelete == false) {
            this.listyst[i].deleshow = true
          } else if (this.listyst[i].checkStart == 1 && this.listyst[i].isCanDelete == true) {
            this.listyst[i].deleshow = false
          }
      ...
      this.$refs['dataForm'].validate(valid => {
        if (valid) {
          if (this.proxyOrgList.length != 0) {
            ...
            this.proxyOrgCode = this.comCode
            this.savDatos()
          }
        }
      })
    ...
    // 新增的保存
    savDatos(row) {
      this.proxyOrgCode = this.temp.proxyOrgCode
      this.fromLoading = true// 弹窗加载动画
      this.temp.list1 = this.smallslist
      this.temp.list2 = this.towlistdata
      createArticlest(this.statusfalse, this.proxyOrgCode, this.temp).then(response => {
        if (response.data.code == 200) {
    ...
    createSubmit() {
      ...
      this.temp.list1 = this.smallslist
      this.temp.list2 = this.towlistdata
      createArticlest(this.statustrue, this.proxyOrgCode, this.temp).then(response => {
        if (response.data.code == 200) {
          this.listyst.unshift(this.temp)
          this.dialogFormVisible = false
          this.getAA()
    ...
    affirmlist() {
      ...
      // this.temp.deliverDate= this.multipleSelection.filingNumber//备案编号
    },
    // 表格的接口
    getListaccount() {
