        ...
        <el-tabs v-model="activeName" v-loading="fromLoading" @tab-click="fathClick"><!-- @tab-click="handleClick" -->
          ...
          <!-- 选项卡表格 -->
            <el-form :rules="rules" ref="dataForm" :model="temp" label-width="110px">
              <el-form-item>
                <span class="equip-title">设备维修验收记录单待办</span>
              </el-form-item>
             ...
             <el-row>
              ...
              <el-form-item label="验收情况" prop="acceptanceInfo" style="margin-left:120px"> 
                <el-radio v-model="temp.acceptanceInfo" :disabled='seSte?true:false' label="1">验收通过</el-radio>
                <el-radio v-model="temp.acceptanceInfo" :disabled='seSte?true:false' label="2">验收不通过</el-radio>
...
<script>
import { fetchList, smallAblist, samllDelete, uploadlist, circuList, updateAuditing, updeletedata, insertFile, getCurrentUser, getNodeTask, newlYupdate } from '@/api/servicingManage/labelsheetTobedone'
...
// ******* 文件预览结束**********
    ...
    checkDetails(row) {
      ...
      if (row.checkStart == 4) {
        ...
        this.seSte = false// 可编辑的框可用
      } else {
        this.consent = true
        this.retrnsty = true
        this.submitlist = false
        this.seSte = true// 可编辑的框不可用
      }
      this.getUser()
    ...
    submData() {
      this.$refs['dataForm'].validate(valid => {
        if (valid) {
          ...
          newlYupdate(this.temp.id, tempData).then(response => {
            const msg = response.data.msg
            if (response.data.code == 200) {
