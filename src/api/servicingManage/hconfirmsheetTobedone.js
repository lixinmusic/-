...
export function fetchList(pageNum, pageSize, data) {
    return request({
        url: MES_EQUIPMENT + "equipment/repair/warranty/getUpcoming" + "?pageNum=" + pageNum + "&pageSize=" + pageSize,
        method: "post",
        data
    });
}
...
export function updateAuditing(query) {
    return request({
        url: MES_EQUIPMENT + "equipment/repair/warranty/operating",
        method: "get",
        params: query
    });
}
...
// 在待办时退回的时候再进行的提交  POST /equipment/repair/warranty/reInitiate
export function newlYupdate(id,data) {
    return request({
        url: MES_EQUIPMENT + "equipment/repair/warranty/reInitiate" + "?id=" + id,
      method: "post",
      data
    });
}
