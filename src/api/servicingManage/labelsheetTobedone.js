...
export function fetchList(pageNum, pageSize, data) {
    return request({
      url: MES_EQUIPMENT + "equipment/repair/acceptance/getUpcoming" + "?pageNum=" + pageNum + "&pageSize=" + pageSize,
      method: "post",
      data
    });
}
...
// 在待办时退回的时候再进行的提交 POST /
export function newlYupdate(id,data) {
    return request({
        url: MES_EQUIPMENT + "equipment/repair/acceptance/reInitiate" + "?id=" + id,
        method: "post",
        data
    });
}
...
// 同意按钮和退回按钮 GET /equipment/repair/acceptance/operating
export function updateAuditing(query) {
    return request({
      url: MES_EQUIPMENT + "equipment/repair/acceptance/operating",
      method: "get",
      params: query
    });
}
